...
@endif
<form action="{{ route('jenis.update', $rs -> id) }}" method="post" enctype="multipart/form-data">
                  {{ csrf_field ()}}
                  {{ method_field ('PUT')}}
    <div class="form-group">
                        <label for="">Nama Jenis</label>
                        @php $val = ($errors->isEmpty()) ? $rs->nama : old('nama'); @endphp
                              <input name="nama" type="text" class="form-control @error('nama') is-invalid @enderror" placeholder="Masukkan Nama Jenis" value="{{ $val }}">
